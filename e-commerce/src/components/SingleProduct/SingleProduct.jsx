...
const SingleProduct = ({
  ...
  return (
    <main>
      <section className="single-product">
        ...
        <section className="product-details">
          ...
          <div className="right-details">
            ...
            <div className="price">
              <button>
                N<span className="price-amount">{findProduct.price}</span>
              </button>
              <p>
            ...
            <p className="product-desc-details">{findProduct.description}</p>
            <div className="quantity-addCart">
              ...
              {itemFoundInCart ? (
                <div>
                  <button onClick={() => handleRemove(itemFoundInCart)}>
                    {' '}
                    -{' '}
                  </button>
                  <span className="productnum">{itemFoundInCart.qty}</span>
                  <button onClick={() => handleAdd(findProduct)}> + </button>
                </div>
              ) : (
                <button onClick={() => handleAdd(findProduct)}>
                  {' '}
                  Add to Cart <CartLogo />{' '}
                </button>
              )}
            </div>
            <div className="rating-details">
              <div>
                <Badge bg="secondary" className="badge-number">
                  {' '}
                  4.5 / 5{' '}
                </Badge>
      ...
      <div className="other-products">SEE OTHER SIMILAR PRODUCTS</div>
    </main>
  )
}
